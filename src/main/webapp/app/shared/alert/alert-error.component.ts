...
export class AlertErrorComponent implements OnDestroy {
  ...
  constructor(private alertService: JhiAlertService, private eventManager: JhiEventManager, translateService: TranslateService) {
    this.errorListener = eventManager.subscribe('iconnectSampleAppLabApp.error', (response: JhiEventWithContent<AlertError>) => {
      const errorResponse = response.content;
      this.addErrorAlert(errorResponse.message, errorResponse.key, errorResponse.params);
    });

    this.httpErrorListener = eventManager.subscribe(
      'iconnectSampleAppLabApp.httpError',
      (response: JhiEventWithContent<HttpErrorResponse>) => {
        ...
        switch (httpErrorResponse.status) {
          ...
          case 400: {
            ...
            if (errorHeader) {
              ...
              for (const fieldError of fieldErrors) {
                ...
                const fieldName = translateService.instant('iconnectSampleAppLabApp.' + fieldError.objectName + '.' + convertedField);
          ...
          case 409:
            this.addErrorAlert('Conflict', 'error.conflict');
            break;
